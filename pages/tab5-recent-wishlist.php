...
error_reporting(E_ALL);
    ...
    $query = $dbconn->prepare("SELECT WISHLIST_PRODUCT.*, PRODUCT.*, SHOP.NAME a, SHOP.IS_VERIFIED b FROM 
                                ...
                                '$id_user' AND WISHLIST_PRODUCT.IS_POST = 0 AND PRODUCT.CODE NOT IN ( SELECT PRODUCT_CODE FROM COLLECTION_PRODUCT WHERE COLLECTION_CODE = '" . $collection_code . "')");
    $query->execute();
    ...
    $query = $dbconn->prepare("SELECT WISHLIST_PRODUCT.*, POST.*, SHOP.NAME a, SHOP.IS_VERIFIED b, POST.TITLE AS NAME, POST.PRICING_MONEY AS PRICE, POST.POST_ID AS CODE FROM 
                                WISHLIST_PRODUCT LEFT JOIN POST ON WISHLIST_PRODUCT.PRODUCT_CODE = POST.POST_ID 
                                LEFT JOIN SHOP ON POST.MERCHANT = SHOP.CODE WHERE FPIN = 
                                '$id_user' AND WISHLIST_PRODUCT.IS_POST = 1 AND POST.POST_ID NOT IN ( SELECT PRODUCT_CODE FROM COLLECTION_PRODUCT WHERE COLLECTION_CODE = '" . $collection_code . "')");
    $query->execute();
            ...
            <div class="row align-items-center" style="background-color: <?= $nexilis_configuration['COLOR_PALETTE'] ?>; padding: 10px 0 30px 0; padding-bottom: 55px">
                ...
                <div id="searchFilter-a" class="col-10 col-md-10 col-lg-10 d-flex align-items-center justify-content-center text-white pl-2 pr-2">
                    <form autocomplete="off" id="searchFilterForm-a" action="search-result" method=GET style="width: 90%;">
                        ...
                        <img class="d-none" id="delete-query" src="../assets/img/icons/X-fill-(Red).png">
                        <img id="voice-search" onclick="voiceSearch()" src="../assets/img/icons/Voice-Command-(Red).png">
                ...
                <?php foreach ($recent_purchase as $recent): ?>
                    ...
                    <div class="row recent-purchase-single-product mt-3">
                        ...
                        <div class="col-2 my-auto">
                            <form action="../logics/tab5/add_product_collection?src=wishlist" method="POST">
