...
<body>
    ...
    <div class="container-fluid section-menu">
        <ul class="nav nav-tabs horizontal-slide gx-0">
            ...
            <li class="nav-item p-2 selected" style="width: 100px; background-color: #e6e6e6" onclick="navPDAM()">
    ...
    <div class="container-fluid section-main">
        <div class="container p-2">
            <div class="row gx-0 mt-3" style="background-color: #f0f3f7; border-radius: 8px; margin-left: 7px; margin-right: 7px; height: 38px">
                <div class="col-12 text-center">
                    ...
                    if ($env == 1) {
                    ?>
                        <div id="pdam" style="margin:4px; background-color: #f06270; color: #FFFFFF; border-radius: 8px; font-size: 15px; height: 30px; padding-top: 3px">Air PDAM</div>
                    <?php
                    } else {
                    ?>
                        <div id="pdam" style="margin:4px; background-color: red; color: #FFFFFF; border-radius: 8px; font-size: 15px; height: 30px; padding-top: 3px">Air PDAM</div>
                    <?php
                    }
                    ?>
                </div>
        ...
        <div class="container p-2">
            <div class="row p-3 mb-2 shadow" style="background-color: #FFFFFF; border-radius: 8px; margin-left: 7px; margin-right: 7px; margin-top: 5px">
                <div class="col-12 d-flex justify-content-start p-2 ms-1">
                    <?php
                    if ($env == 1) {
                    ?>
                        <h1 class="mb-0" style="font-size: 18px; font-weight: 600">Bayar tagihan PDAM Anda di <span><img src="../assets/img/Virtoo Assets-20220420T065146Z-001/GTM 2.0-Virtoo Assets/Logo/Virtoo_Gradient.png" alt="" style="height: 70px; width: 70px; margin-top: -3px; margin-left: -5px"></span></h1>
                    <?php
                    } else {
                    ?>
                        <h1 class="mb-0" style="font-size: 18px; font-weight: 600">Bayar tagihan PDAM Anda di <span style="font-size: 20px; font-weight: 700; color: red">DigiPos</span></h1>
                    <?php
                    }
                    ?>
                </div>
                <div class="col-12">
                    ...
                    <select id="pdam-wilayah" class="form-select mt-3" aria-label="Default select example">
                        <option value="" selected>Pilih Wilayah</option>

                        <?php foreach ($wilayah as $w) : ?>

                            <option value="<?= $w['CODE'] ?>"><?= $w['DESCRIPTION'] ?></option>

                        <?php endforeach; ?>

                    </select>

                    <input type="number" pattern="\d*" class="form-control mt-3 mb-1" id="pdam-number" placeholder="No. Pelanggan" aria-describedby="emailHelp">

                    <div id="btn-pdam-bayar" class="btn btn-light mt-3 mb-2" style="width: 100%; color: #cacaca; border: 1px solid #cacaca" onclick="pdamCheck()">Cek Tagihan</div>
                </div>
            </div>
        </div>
    </div>

    <div class="container-fluid" id="section-purchase" style="margin-top: 240px">
            ...
            <div class="col-12 d-flex justify-content-center">
                <div class="row gx-0 p-3" style="width: 100%; margin-top: -15px">
                    ...
                    <div class="col-2 d-flex justify-content-end">
                        ...
                        if ($env == 1) {
                        ?>
                            <button class="btn text-light" style="background-color: #f06270; margin-top: 160px" onclick="purchasePDAM()">Bayar</button>
                        <?php
                        } else {
                        ?>
                            <button class="btn text-light" style="background-color: red; margin-top: 160px" onclick="purchasePDAM()">Bayar</button>
                        <?php
                        }
                        ?>
                    </div>
    ...
    <footer id="footer" style="border-top: 1px solid #d7d7d7; margin-top: 240px; margin-bottom: 120px">
...
<script>
    ...
    $("#pdam-number").bind("change paste keyup", function() {
        var number = $(this).val();
        var wilayah = $("#pdam-wilayah").val();

        if (number && wilayah != "") {

            $('#btn-pdam-bayar').attr('class', 'btn text-light mt-3 mb-2');
            <?php
            if ($env == 1) {
            ?>
                $('#btn-pdam-bayar').css('background-color', '#f06270');
                $('#btn-pdam-bayar').css('border', '1px solid #f06270');
            <?php
            } else {
            ?>
                $('#btn-pdam-bayar').css('background-color', 'red');
                $('#btn-pdam-bayar').css('border', '1px solid red');
            <?php
            }
            ?>
            ...
            $('#btn-pdam-bayar').attr('class', 'btn btn-light mt-3 mb-2');
            $('#btn-pdam-bayar').attr('style', 'background-color: none; width: 100%');
            $('#btn-pdam-bayar').css('color', '#cacaca');
            $('#btn-pdam-bayar').css('border', '1px solid #cacaca');

        }

    });

    $("#pdam-wilayah").change(function() {

        var wilayah = $(this).val();
        var number = $("#pdam-number").val();
    ...
    function pdamCheck() {
        ...
        var command = "INQUIRY";
        var product = $('#pdam-wilayah').val();
        var customer = $('#pdam-number').val();
