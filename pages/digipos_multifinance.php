...
$sqlData = "SELECT * FROM PPOB_PRODUCT WHERE TYPE = 14";

$queDATA = $dbconn->prepare($sqlData);
$queDATA->execute();
$finance = $queDATA->get_result();
$queDATA->close();
...
<body>
    ...
    <div class="section-menu">
        <ul class="nav nav-tabs horizontal-slide gx-0">
            ...
            <li class="nav-item p-2 selected" style="width: 100px; background-color: #e6e6e6" onclick="navMultifinance()">
    ...
    <div class="row gx-0" style="height: 430px">
        <div class="col-12 justify-content-center">
            <div class="section-main">
                <div class="container p-2">
                    <div class="row gx-0 mt-3" style="background-color: #f0f3f7; border-radius: 8px; margin-left: 7px; margin-right: 7px; height: 38px">
                        <div class="col-12 text-center" onclick="navKesehatan()">
                            ...
                            if ($env == 1) {
                            ?>
                                <div id="bpjs" style="margin:4px; background-color: #f06270; color: #FFFFFF; border-radius: 8px; font-size: 15px; height: 30px; padding-top: 3px">Multifinance</div>
                            <?php
                            } else {
                            ?>
                                <div id="bpjs" style="margin:4px; background-color: red; color: #FFFFFF; border-radius: 8px; font-size: 15px; height: 30px; padding-top: 3px">Multifinance</div>
                            <?php
                            }
                            ?>
                        </div>
                ...
                <div class="container p-2">
                    <div class="row p-3 mb-2 pt-4 shadow" style="background-color: #FFFFFF; border-radius: 8px; margin-left: 7px; margin-right: 7px; margin-top: 5px">
                        <b>Pilih Tagihan</b>
                        <div class="col-12 mt-4">
                            <select id="multifinance-select" style="margin-top: -10px; border: none; border-bottom: 2px solid var(--color-border,#E5E7E9)" class="form-select" aria-label="Default select example">

                                <option value="">Jenis Pembayaran</option>

                                <?php foreach ($finance as $f) : ?>
                                    <option value="<?= $f['CODE'] ?>"><?= $f['DESCRIPTION'] ?></option>
                                <?php endforeach; ?>
                        ...
                        <div class="col-12 d-flex justify-content-start p-2 ms-1 mt-3">
                            <h6 class="mb-0" style="font-weight: 600">Bayar Cicilan Kredit ACC Finance</h6>
                        </div>

                        <!-- MULTIFINANCE -->
                        <div id="pstn" class="col-12 mt-2">
                            <div class="row">
                                <div class="col-12">
                                    <p class="mb-0" style="font-size: 13px; font-weight: 400; color: rgb(136, 136, 136); margin-left: 3px">Masukkan Nomor Kontrak / ID Pelanggan</p>
                                </div>
                                <div class="col-12 mt-2 mb-3">
                                    <input id="nomorhp" type="number" pattern="\d*" placeholder="081234567XXX" style="border: none; border-bottom: 2px solid var(--color-border,#E5E7E9); width: 100%">
                                </div>
                            </div>
                            <div class="row mt-3 mb-3">
                                <div class="col-12">
                                    ...
                                    if ($env == 1) {
                                    ?>
                                        <button onclick="multifinanceCheck()" id="bayar" class="btn" style="width: 100%; border: 1px solid #cacaca; color: white; background-color: #f06270">Cek Tagihan</button>
                                    <?php
                                    } else {
                                    ?>
                                        <button onclick="multifinanceCheck()" id="bayar" class="btn" style="width: 100%; border: 1px solid #cacaca; color: white; background-color: red">Cek Tagihan</button>
                                    <?php
                                    }
                                    ?>
                                </div>
        ...
        <div class="col-12 d-flex justify-content-center">
            <div class="row gx-0 p-3" style="width: 100%; margin-top: -15px">
                ...
                <div class="col-2 d-flex justify-content-end" style="position:relative">
                    ...
                    if ($env == 1) {
                    ?>
                        <button class="btn text-light" style="background-color: #f06270;  position:absolute; bottom:0;" onclick="purchaseMultifinance()">Bayar</button>
                    <?php
                    } else {
                    ?>
                        <button class="btn text-light" style="background-color: red;  position:absolute; bottom:0;" onclick="purchaseMultifinance()">Bayar</button>
                    <?php
                    }
                    ?>
                </div>
...
<script>
    ...
    $("#nomorhp").bind("change space keyup", function() {
        var nohp = $(this).val();
        var multi = $("#multifinance-select").val();
    ...
    $("#multifinance-select").change(function() {
        var multi = $(this).val();
        var nohp = $("#nomorhp").val();
    ...
    function multifinanceCheck() {
        ...
        var command = "INQUIRY";
        var product = $('#multifinance-select').val();
