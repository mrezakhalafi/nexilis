...
$sqlData = "SELECT * FROM PPOB_PRODUCT WHERE TYPE = 10";

$queDATA = $dbconn->prepare($sqlData);
$queDATA->execute();
$token = $queDATA->get_result();
$queDATA->close();
...
<body>
    ...
    <div class="container-fluid section-menu">
        <ul class="nav nav-tabs horizontal-slide gx-0">
            ...
            <li class="nav-item p-2 selected" style="width: 100px; background-color: #e6e6e6" onclick="navPLN()">
        ...
        <div class="container p-2">
            <div class="row gx-0 mt-3" style="background-color: #f0f3f7; border-radius: 8px; margin-left: 7px; margin-right: 7px; height: 38px">
                <div class="col-12 text-center">
                    ...
                    if ($env == 1) {
                    ?>
                        <div id="pln" style="margin:4px; background-color: #f06270; color: #FFFFFF; border-radius: 8px; font-size: 15px; height: 30px; padding-top: 3px">PLN</div>
                    <?php
                    } else {
                    ?>
                        <div id="pln" style="margin:4px; background-color: red; color: #FFFFFF; border-radius: 8px; font-size: 15px; height: 30px; padding-top: 3px">PLN</div>
                    <?php
                    }
                    ?>
                </div>
        ...
        <div class="container p-2">
            <div class="row p-3 mb-2 shadow" style="background-color: #FFFFFF; border-radius: 8px; margin-left: 7px; margin-right: 7px; margin-top: 5px">
                <div class="col-12">
                    ...
                    <div class="row mt-2" style="font-size: 10px; font-weight: 700">
                        <div class="col-4">
                            <div class="form-check">
                                <input class="form-check-input" name="product" type="radio" value="Token" id="flexRadioDefault" checked>
                        ...
                        <div class="col-4">
                            <div class="form-check">
                                <input class="form-check-input" name="product" type="radio" value="PLN" id="flexRadioDefault2">
                        ...
                        <div class="col-4">
                            <div class="form-check">
                                <input class="form-check-input" name="product" type="radio" value="PLNNON" id="flexRadioDefault3">
                    ...
                    <input type="number" pattern="\d*" class="form-control mt-3 mb-1" id="pln-number" placeholder="No. Meter / ID Pelanggan" aria-describedby="emailHelp">

                    <select id="token-select" class="form-select mt-3" aria-label="Default select example">
                        <option value="" selected>Pilih Jumlah Token</option>
                    ...
                    <div class="pln-notes p-3 mt-4 mb-2" style="border: 1px solid #f06270; font-size: 12px; background-color: #fff2f2">
                        <b>Keterangan</b><br><br>
                        <p>1. Transaksi Produk Listrik PLN yang dilakukan pukul 23:40-00:45 akan mulai diproses pada pukul 00:45 WIB sesuai kebijakan pihak PLN</p>
                        <p>2. Proses verifikasi transaksi maksimal 2x24 jam hari kerja.</p>
                        <p>3. Harap cek limit kWh anda sebelum membeli token listrik</p>
                    </div>

                    <div id="btn-pln-bayar" class="btn btn-light mt-3 mb-2" style="width: 100%; color: #cacaca; border: 1px solid #cacaca" onclick="plnCheck()">Beli Token</div>
                </div>
            </div>
        </div>
    </div>

    <div class="container-fluid" id="section-purchase" style="margin-top: 450px">
        <div  class="row gx-0 p-3">
            ...
            <div id="token-purchase" class="col-12 d-flex justify-content-center d-none">
                <div class="row gx-0 p-3" style="width: 100%; margin-top: -15px">
                    ...
                    <div class="col-2">
                        ...
                        if ($env == 1) {
                        ?>
                            <button class="btn text-light" style="background-color: #f06270; margin-top: 160px" onclick="purchasePln()">Bayar</button>
                        <?php
                        } else {
                        ?>
                            <button class="btn text-light" style="background-color: red; margin-top: 160px" onclick="purchasePln()">Bayar</button>
                        <?php
                        }
                        ?>
                    </div>
            ...
            <div id="non-token-purchase" class="col-12 d-flex justify-content-center d-none">
                <div class="row gx-0 p-3" style="width: 100%; margin-top: -15px">
                    <div class="col-6 d-flex justify-content-start">
                        <div class="row">
                            ...
                            <div class="col-12">
                                <p id="price-purchase-token" class="ms-1 mb-0" style="font-size: 14px; font-weight: 700">Rp 0</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-6 d-flex justify-content-end">
                        ...
                        if ($env == 1) {
                        ?>
                            <button style="background-color: #f06270" class="btn text-light" onclick="purchasePlnToken()">Bayar</button>
                        <?php
                        } else {
                        ?>
                            <button style="background-color: red" class="btn text-light" onclick="purchasePlnToken()">Bayar</button>
                        <?php
                        }
                        ?>
                    </div>
    ...
    <footer id="footer" style="border-top: 1px solid #d7d7d7; margin-top: 450px; margin-bottom: 120px">
...
<script>
    ...
    $("#pln-number").bind("change paste keyup", function() {
        var number = $(this).val();
        var choice = $('#token-select').val();
        ...
        if (radio != "Token") {
            ...
            if (number) {
                ...
                $('#btn-pln-bayar').attr('class', 'btn text-light mt-3 mb-2');
                <?php
                if ($env == 1) {
                ?>
                    $('#btn-pln-bayar').css('background-color', '#f06270');
                    $('#btn-pln-bayar').css('border', '#f06270');
                <?php
                } else {
                ?>
                    $('#btn-pln-bayar').css('background-color', 'red');
                    $('#btn-pln-bayar').css('border', 'red');
                <?php
                }
                ?>
                ...
                $('#btn-pln-bayar').attr('class', 'btn btn-light mt-3 mb-2');
                $('#btn-pln-bayar').attr('style', 'background-color: none; width: 100%');
                $('#btn-pln-bayar').css('color', '#cacaca');
                $('#btn-pln-bayar').css('border', '1px solid #cacaca');
    ...
    $('input[type=radio][name=product]').change(function() {
        ...
        if (this.value == 'PLN' || this.value == 'PLNNON') {
            $('#token-select').hide();
            $("#btn-pln-bayar").text("Cek Tagihan");
        } else {
            $('#token-select').show();
            $("#btn-pln-bayar").text("Beli Token");
        }
    ...
    $('#token-select').change(function() {
        ...
        price = $('#token-select').text().match(/\d+/);
        $('#price-purchase-token').text("Rp " + price);
    ...
    function plnCheck() {
        ...
        var checkNoMeter = $('#pln-number').val();
        var checkProduct = $("input[type='radio']:checked").val();
        var pass = 0;

        if (checkProduct == 'Token') {
            var amountToken = $('#token-select').val();
        ...
        if (checkNoMeter && pass == 1) {

            $('#section-purchase').show();

            var command = "INQUIRY";
            var product = $("input[type='radio']:checked").val();
            var customer = $('#pln-number').val();
            ...
            if (type == "PLN" || type == "PLNNON") {

                $('#non-token-purchase').addClass('d-none');
                $('#token-purchase').removeClass('d-none');
                $('#footer').css('margin-top', '0px');
                vbotAPI(digipos_cart);

            } else {

                $('#token-purchase').addClass('d-none');
                $('#non-token-purchase').removeClass('d-none');
                $('#footer').css('margin-top', '0px');
    ...
    function purchasePlnToken() {

        var command = "PURCHASE";
        var product = $("#token-select").val();
        var customer = $('#pln-number').val();
        var partner_trxid = new Date().getTime().toString(16);
        var payment = $('#dropdownMenuSelectMethod').val();
