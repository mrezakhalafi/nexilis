    ...
    if (saved.length > 0) {
        ...
        saved.slice().reverse().forEach(item => {
            ...
            if (localStorage.lang == 0) {
                ...
                viewstore = "Kunjungi Profil";
            }

            if (item.official_acc == 1 || item.official_acc == 3) {
                icon_verified = '<img class="verified" src="../assets/img/ic_official_flag.webp">';
            } else if (item.official_acc == 2) {
                icon_verified = '<img class="verified" src="../assets/img/ic_verified_flag.png">';
            }
            ...
            let html_shop =
                ...
                `<div class="col-2">${icon_verified}</div><div class="col-10 px-1">${merchant_name}</div>` +
                '</div>' +
                '</div>' +
            ...
            item.items.forEach(item => {
                ...
                let html_item =
                    ...
                    `<div class="text-grey" onclick="checkItem('${item.itemName}', 'items')">` + move + `</div>` +
                    '</div>' +
                    '</div>' +
                    '</div>' +
                    '</div>' +
...
var cardModalHtml =
    ...
    '  </div>' +
    '  <div class="row mb-2">' +
    '    <input maxlength="16" size="16" type="text" required class="form-control" id="credit-card-number" placeholder="e.g: 4000000000001091" name="creditCardNumber">' +
    '  </div>' +
    '  <div class="row mb-4">' +
    '    <div class="col-3">' +
    '  <div class="row">' +
...
var shopeepayModalHtml =
    ...
    '   <div class="col p-3">' +
    '       <div class="row">' +
    '           <input class="pay-button" onclick="return toSubmitSHOPEE();" type="submit" id="pay-with-shopeepay" class="col-md-12 simple-modal-button-green py-1 px-3 m-0 my-4 fs-16" value="Pay" name="payWithSHOPEEPAY">' +
    '       </div>' +
    '   </div>' +
    '</fieldset>' +
    '</form>';
...
var qrisModalHtml =
    ...
    '       <div class="row">' +
    '           <div id="qrcode"></div>' +
    '           <input class="pay-button" onclick="return toSubmitQRIS();" type="submit" id="pay-with-qris" class="col-md-12 simple-modal-button-green py-1 px-3 m-0 my-4 fs-16" value="Generate QR Code" name="payWithQRIS">' +
    '           <br><button type="button" class="pay-button mt-3 d-none" id="simulate-qris-payment">Pay QRIS</button>' +
    '       </div>' +
    '   </div>' +
    '</fieldset>' +
    '</form>';
        ...
        function (data, status) {
            try {
                if (data == "SUCCEEDED") {
                    ...
                    postForm("../logics/paliobutton/php/store_payment", {
                        fpin: fpin,
                        method: "OVO",
                        status: "success",
                        items: base64items,
                        cart: btoa(JSON.stringify(purchased_cart))
                    });
                } else {
...
function toSubmitDANA() {
    ...
    var js = {
        ...
        callback: window.location.origin + "/nexilis/pages/checkout.php?f_pin=" + fpin,
        amount: localStorage.getItem('grand-total'),
    };
    ...
    xhr.onload = async function () {
        if (xhr.status != 200) { // analyze HTTP status of the response
            ...
            if (responseObj.status == "COMPLETED") {
                ...
                var method = responseObj.payment_detail.source;
                clearCart();
                var items = JSON.stringify(cart);
                var base64items = btoa(items);
                var fpin = getFpin();
...
function toSubmitQRIS() {
    ...
    var js = {
        // callback: this.callbackURL,
        // callback: "http://202.158.33.26/paliobutton/php/close",
        callback: window.location.origin + "/nexilis/pages/payment.php?f_pin=" + fpin,
        amount: localStorage.getItem('grand-total'),
    };
    ...
    $.post("../logics/paliobutton/php/paliopay_qris",
        ...
        function (data, status) {
            try {
                var response = JSON.parse(data);
                console.log("QRIS", response);

                new QRCode(document.getElementById('qrcode'), response.qr_string);

                runSimulateQRIS(response.external_id);

                // checkQRISStatus(response.id);
                // simulateQRISPayment(response.external_id);

                // window.open(response.actions.desktop_web_checkout_url, "_blank");
                // window.open(response.actions.mobile_deeplink_checkout_url,"_blank");
                // console.log(response.actions.desktop_web_checkout_url);
            } catch (err) {
                console.log("QRIS ERR",err);
                alert("Error occured");
...
function toSubmitLINKAJA() {
    ...
    var js = {
        // callback: this.callbackURL,
        // callback: "http://202.158.33.26/paliobutton/php/close",
        callback: window.location.origin + "/nexilis/pages/checkout.php?f_pin=" + getFpin(),
        amount: localStorage.getItem('grand-total'),
    };
...
function toSubmit() {
    ...
    // OCT 2022 xnd_development_1aoXe1u4OVKIxcWuJdAXDzvs8kvAep8CDRYstM49JpfNSNqTcMxUVcP5iH59ix
    // SELF NOV2022 xnd_public_development_JO6jvFM5bOjNguUB49cjZAQ9A36ArCEjDGx3kp7QxEwZsL4vmG1uVH1gREAHmZFt
...
function xenditResponseHandler(err, creditCardCharge) {
    ...
    console.log("XENDIT ERR", err);
    console.log("XENDIT ReSP", creditCardCharge);
...
async function changeDeliveryAddress() {
    ...
    let fpin = new URLSearchParams(window.location.search).get('f_pin')
    window.open('/nexilis/pages/tab5-change-address?f_pin=' + fpin, '_self')
}
