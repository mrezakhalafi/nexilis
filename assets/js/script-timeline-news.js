...
function fetchProductCount(store_id, prev_STORE_ID) {
  ...
  xmlHttp.onreadystatechange = function () {
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
      let data = JSON.parse(xmlHttp.responseText);
      // if (data != null && data.PRODUCT_COUNT > 0) {
...
async function searchFilter() {
  ...
  var product_dest = "timeline_news.php";
  var filter_dest = "timeline_story_container_timeline.php";
  var params = "";
  ...
  isCalled = false;
  countVideoPlaying = 0;
  await displayRecords(params, 5, offset);
...
function drawGIFs(arr) {
  ...
  if (pickGif.URL.includes('bni.co.id')) {
    ...
    } else if (path.includes("tab1-main-news")) {
      if (currURL.includes('newuniverse.io')) {
