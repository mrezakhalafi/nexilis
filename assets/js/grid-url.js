...
// to get merchants that have products --> REPLACE FUNCTION NON EMPTY MERCHANTS HERE

// to shuffle product order in tab 1 --> REPLACE FUNCTION NON EMPTY MERCHANTS HERE
    ...
    if (thumb_arr.length > 1) {
      thumb_arr.forEach((image, jIdx) => {
        if (image.length > 0) {
          ...
          let linkProfile = element.LINK_URL;
          let linkImg = element.LINK_ID;
          let linkURL = element.LINK_URL;

          if ($image_type_arr.includes(fileExt)) {
            imgElem = '<img onclick="openProfile(\'' + linkProfile + '\')" id="' + element.LINK_ID + '" class="content-image" src="' + domain + '/nexilis/images/' + image.trim() + '" loading="lazy"/> <img src="/nexilis/assets/img/ic_more.png" width="25" height="25" style="filter: invert(1); float: right; z-index: 999" onclick="dropDownMenu(`'+linkImg+'`, `'+linkImg+'`, `'+linkURL+'`)"/>'
      ...
      if ($image_type_arr.includes(fileExt)) {
        imgElem = '<img onclick="openProfile(\'' + linkProfile + '\')" class="content-image" src="' + domain + '/nexilis/images/' + thumb_arr[0].trim() + '" loading="lazy"/> <img src="/nexilis/assets/img/ic_more.png" width="25" height="25" style="filter: invert(1); float: right; z-index: 999" onclick="dropDownMenu(`'+linkImg+'`, `'+linkImg+'`, `'+linkURL+'`)"/>'
      } else if ($video_type_arr.includes(fileExt)) {
        var isAutoplay = size == 2 ? 'autoplay' : '';
        imgElem = '<video muted loop playsinline class="content-image" id="video-' + element.LINK_ID + '" src="' + domain + '/nexilis/images/' + thumb_arr[0].trim() + '#t=5" type="video/' + fileExt + '"></video>';
      }
      contents = imgElem;
    }
...
function fillGridWidgets(grid, sort_by, lim, off) {
  ...
  batch.forEach((element, idx) => {
    ...
    let thumb_ext = ext(thumb_id).substr(1);
    // console.log("thumb_ext: ", thumb_ext);
    ...
    // console.log("thumb_arr: ", thumb_arr);
    ...
    if (thumb_arr.length > 1) {
      thumb_arr.forEach((image, jIdx) => {
        if (image.length > 0) {
          ...
          if ($image_type_arr.includes(fileExt)) {
            imgElem = '<img onclick="openProfile(\'' + linkProfile + '\')" class="content-image" src="' + domain + '/nexilis/images/' + image.trim() + '" loading="lazy"/> <img src="/nexilis/assets/img/ic_more.png" width="25" height="25" style="filter: invert(1); float: right; z-index: 999" onclick="dropDownMenu(`'+linkImg+'`, `'+linkImg+'`, `'+linkURL+'`)"/>'
          } else if ($video_type_arr.includes(fileExt)) {
            var isAutoplay = size == 2 ? 'autoplay' : '';
            imgElem = '<video preload="metadata" muted loop playsinline class="content-image" id="video-' + element.LINK_ID + '" src="' + domain + '/nexilis/images/' + image.trim() + '#t=5" type="video/' + fileExt + '"></video>';
          }
...
function fetchLinks() {
  ...
  xmlHttp.open("get", "/nexilis/logics/fetch_grid_url?f_pin=" + f_pin);
...
// --> ISINYA async function changeSort(sort) {
// --> ISINYA $('#sort-store-popular').click(async function () {
// --> ISINYA $('#sort-store-date').click(async function () {
// --> ISINYA $('#sort-store-follower').click(async function () {
...
function dropDownMenu(linkProfile, linkImg, linkURL) {

  console.log('+++',linkProfile);

  for (var i=0; i<dataFiltered.length; i++){

    if (dataFiltered[i].LINK_ID == linkProfile){
     
      var img_name = dataFiltered[i].THUMB;
      var linkImg = linkImg;

      var html = `
      <form method="POST" id="edit-url" class="main-form">
        <div class="row">
          <div class="col-12">
            <label for="modify-image" style="width: 100%">
              <img id="img-modal" src="../images/`+img_name+`" alt="" style="width: 100%">
            </label>
            <input id="modify-image" name="modify-image" type="file" class="d-none" onchange="imgModal()">
            <img src="../assets/img/ic_more.png" style="width: 50px; height: 50px; position: absolute; right: 0" id="ddMenuURL" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            <ul class="dropdown-menu" aria-labelledby="ddMenuURL">
              <li><a class="dropdown-item" href="#" onclick="saveURL('`+linkImg+`')">Save URL</a></li>
              <li><a class="dropdown-item text-danger" href="#" onclick="deleteURL(`+linkImg+`)">Delete URL</a></li>
            </ul>
          </div>
          <div class="col-12 p-3">
            <input id="url-name" type="text" class="form-control form-control-lg" style="width: 100%" placeholder="Link URL Name" value="`+linkURL+`">
          </div>
        </div>
      </form>`;

      $("#modal-url-body").html(html);
      console.log("lp_dd: ", linkImg);
