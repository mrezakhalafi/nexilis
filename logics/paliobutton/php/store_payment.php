...
$method = $_POST['method'];
$status = $_POST['status'];
$transaction_id = md5(date('Y-m-d H:i:s') . $f_pin);
...
foreach ($cart as $c) {
  ...
  if (isset($c->isPost)) {
    ...
    $query->bind_param("sssiiss", $transaction_id, $merchant_id, $p_code, $price, $amount, $method, $f_pin);
    $query->execute();
    $query->close();
  } else {
    $query = $dbconn->prepare("INSERT INTO PURCHASE (TRANSACTION_ID, MERCHANT_ID, PRODUCT_ID, POST_ID, PRICE, AMOUNT, METHOD, FPIN) VALUES (?, ?, ?, NULL, ?, ?, ?, ?)");
...
$f_pin_shop_owner = $query_shop_owner->get_result()->fetch_assoc()['CREATED_BY'];
$query_shop_owner->close();

$type = '2';
$time = time() * 1000;
$notif_id_user = $f_pin . $time;
$notif_id_merchant = $f_pin_shop_owner . $time;
$query = $dbconn->prepare("INSERT INTO USER_NOTIFICATION (NOTIF_ID, TYPE, F_PIN, ENTITY_ID, TIME) VALUES (?, ?, ?, ?, ?)");
$query->bind_param("ssssi", $notif_id_user, $type, $f_pin, $transaction_id, $time);
$query->execute();
$query->close();

$query = $dbconn->prepare("INSERT INTO USER_NOTIFICATION (NOTIF_ID, TYPE, F_PIN, ENTITY_ID, TIME) VALUES (?, ?, ?, ?, ?)");
$query->bind_param("ssssi", $notif_id_merchant, $type, $f_pin_shop_owner, $transaction_id, $time);
$query->execute();
$query->close();

// echo "success";
header("location: /nexilis/pages/payment.php?f_pin=" . $f_pin . "&id=" . $transaction_id, true, 301);
